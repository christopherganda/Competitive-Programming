...
int lazy[2*100] = {}; // Menyimpan data lazy dari tree
...
void updateLazy(int start,int end,int L,int R,int value,int indeks) {
     if (!(start > R || end < L))  {
        if (L <= start && end <= R) {
           tree[indeks] += value;
           if (start != end) lazy[indeks] += value;
        }
        else {
             int mid = (start + end) / 2;
             if (L <= mid) updateLazy(start, mid, L, R, value, indeks*2);
             if (R >= mid + 1) updateLazy(mid+1, end, L, R, value, indeks*2 + 1);
...
int query(int start,int end,int L,int R,int indeks) {
    if (L <= start && end <= R) return tree[indeks];
         ...
         int mid = (start + end) / 2;
         tree[indeks*2] += lazy[indeks];
         tree[indeks*2 + 1] += lazy[indeks];
         if (start != mid) lazy[indeks*2] += lazy[indeks];
         if (mid + 1 != end) lazy[indeks*2 + 1] += lazy[indeks];
         lazy[indeks] = 0;
...
int main()
{
    ...
    scanf("%d",&Q);
    rep(i,1,Q) {
       printf("Tipe query (1 : Update range, 2 : Max) : ");
       scanf("%d",&T);
       if(T == 1) {
